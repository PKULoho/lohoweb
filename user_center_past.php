            ...
            <div class="side-nav-modules">
                <ul class="module-list">
                    ...
                    <a href="user_center_past.php">
                        <li class="module-item active">过往活动</li>
                    </a>
        ...
        <div class="right-area">
            <div class="right-section">
                ...
                <div class="collection-items">
                    ...
                    $sqlstr = 'SELECT * FROM joined_act, act_info, act_subject WHERE joined_act.joined_act_id = act_info.act_id AND act_subject.act_id = act_info.act_id AND joined_act.user_name = "' . $_COOKIE['username'] . '"';
            ...
            <div class="right-section">
                ...
                <div class="collection-items">
                    <?php
                    $overduestr = 'SELECT * FROM applied_act, act_info, act_subject WHERE applied_act.applied_act_id = act_info.act_id AND act_subject.act_id = act_info.act_id AND act_info.act_first_day < curdate() AND applied_act.user_name = "' . $_COOKIE['username'] . '" AND NOT EXISTS(SELECT * FROM joined_act WHERE joined_act.joined_act_id = applied_act.applied_act_id AND joined_act.user_name = "' . $_COOKIE['username'] . '")';
                    //echo $overduestr;
                    $result = mysqli_query($dbc, $overduestr);
                    while ($row = mysqli_fetch_array($result)) {
                        create_item_overdue($dbc, $row);
                    }

                    function create_item_overdue($dbc, $row)
                    {
                        $act_id = $row['act_id'];
                        $act_name = $row['act_name'];
                        $act_time = $row['act_time'];
